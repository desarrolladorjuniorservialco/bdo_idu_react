...
export function SeccionDiario({ items, total }: Props) {
  return (
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[12px] mt-0.5" style={{ color: 'var(--text-muted)' }}>
              Actividades ejecutadas en obra durante el período
            </p>
          </div>
        </div>
        <span
        ...
        <>
          ...
          {total > MAX && (
            <p className="text-xs mt-2 text-right" style={{ color: 'var(--text-muted)' }}>
              Mostrando {MAX} de {total} — ver página completa en Anotaciones Diario.
            </p>
          )}
        </>
      )}
