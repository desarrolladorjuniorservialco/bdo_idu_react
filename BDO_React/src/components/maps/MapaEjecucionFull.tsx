...
function parseLL(r: any): [number, number] | null {
  const lat = parseFloat(r.latitud);
  const lon = parseFloat(r.longitud);
  return isNaN(lat) || isNaN(lon) ? null : [lat, lon];
}

function InfoPopup({ r, label }: { r: any; label: string }) {
  const color = ESTADO_COLOR[r.estado] ?? '#555';
  return (
    <div style={{ minWidth: 190, fontSize: 12, lineHeight: 1.7 }}>
      <b style={{ color }}>{label}</b>
      {r.folio            && <div><b>Folio:</b> {r.folio}</div>}
      {r.estado           && <div><b>Estado:</b> {r.estado}</div>}
      {(r.id_tramo ?? r.tramo)
                          && <div><b>Tramo:</b> {r.id_tramo ?? r.tramo}</div>}
      {r.civ              && <div><b>CIV:</b> {r.civ}</div>}
      {(r.tipo_actividad ?? r.actividad)
                          && <div><b>Actividad:</b> {r.tipo_actividad ?? r.actividad}</div>}
      {r.tipo_componente  && <div><b>Componente:</b> {r.tipo_componente}</div>}
      {r.item_pago        && <div><b>Ítem:</b> {r.item_pago}</div>}
      {r.observaciones    && <div><b>Obs:</b> {String(r.observaciones).slice(0, 120)}</div>}
      {r.descripcion      && <div><b>Desc:</b> {String(r.descripcion).slice(0, 120)}</div>}
      {r.cantidad != null && <div><b>Cantidad:</b> {r.cantidad} {r.unidad ?? ''}</div>}
      {(r.fecha ?? r.fecha_reporte ?? r.fecha_creacion)
                          && <div><b>Fecha:</b> {r.fecha ?? r.fecha_reporte ?? r.fecha_creacion}</div>}
    </div>
  );
}

function PointLayer({ records, label }: { records: any[]; label: string }) {
  return (
    <>
      {records.map((r, i) => {
        const ll = parseLL(r);
        ...
        return (
          <CircleMarker
            ...
            fillOpacity={0.85}
          >
            <Popup><InfoPopup r={r} label={label} /></Popup>
          </CircleMarker>
        );
      })}
    </>
...
export default function MapaEjecucionFull({
  ...
  const allGeo = [...cantidades, ...componentes, ...reporteDiario, ...formularioPmt]
    .map(r => parseLL(r))
