...
export async function fetchPresupuesto(contratoId: string) {
  ...
  const [{ data: items }, { data: tramos }] = await Promise.all([
    ...
    supabase
      .from('tramos_bd')
      .select('id, id_tramo, nombre, contrato_id, infraestructura, meta_fisica, ejecutado, und, cicloruta_km, esp_publico_m2, tramo_descripcion, avance_pct, estado_ejecucion')
