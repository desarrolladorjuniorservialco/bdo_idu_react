...
export default function AnotacionesDiario({ perfil }) {
  ...
  async function loadFotos(folio) {
    ...
    const { data } = await supabase.from('rf_reporte_diario').select('*').eq('folio', folio);
  ...
  return (
    <div>
      <SectionBadge label="Reporte Diario de Obra" color="purple" />
      <h3>Registros de Actividad Diaria</h3>
      ...
      <div className="filter-form-wrap">
        ...
        <div className="filter-grid filter-grid-4" style={{ marginBottom: '0.75rem' }}>
          ...
          <div className="form-group">
            ...
            <input type="text" className="form-input" placeholder="Folio / tramo / actividad..."
